...
const hasChartData = computed(() => props.data.datasets.some((dataset) => Array.isArray(dataset.data) && dataset.data.length > 0));
...
<template>
  <div class="bet-board-chart" :class="{'is-empty': shouldShowEmpty}" :style="chartStyle">
    <canvas v-show="!shouldShowEmpty" ref="canvasElement" :aria-label="chartLabel" role="img" />
